...
int asciiSmaller(char str1[], char str2[]){//função criada pelo aluno #ok
  //retorna 1 se str1 vem antes na ordem alfabética
  //caso contrário retorna 0

  int i1, i2;

  for(i1 = i2 = 0; str1[i1] && str2[i2]; i1++, i2++){
    if(str1[i1] < str2[i2]) return 1;
    else if (str1[i1] > str2[i2]) return 0;
    //se forem iguais continua o loop
  }

  //se forem iguais até o fim que tiver menor comprimento,
  //o com maior comprimento é considerado à frente na
  //ordem alfabética
...
int partitionVetor(TpContato vetor[], int p, int q) {
  ...
  */
  for(i = p - 1, j = p; j < q; j++){
    if(asciiSmaller(vetor[j].nome, pivo.nome)){
      i++;
...
TpContato * partitionLista(TpContato *p, TpContato *q) {
  ...
  */
  ...
  for(i = NULL, j = p; i != p && j != p; j = j->prox){
    if(asciiSmaller(j->nome, pivo->nome)){
      i = i == NULL ? p : i->prox;
      swap(i, j);
    }
  }
  
  swap(i == NULL ? p : i->prox, q);
  return i->prox;
}
